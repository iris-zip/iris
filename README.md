...
End-to-end encrypted, post-quantum-hybrid, ephemeral text and file transfer. Pair two devices with a 5-digit code, type or drop a file, done. No accounts, no logs, no persistence.
...
Beem is the opposite: nothing is persisted, the relay is blind, and the session key is hybrid — an attacker must break SPAKE2 *and* X25519 *and* ML-KEM-768 to read a transcript, classical or quantum.
...
- Traffic-analysis-based deanonymization (Beem is not an anonymity tool; use Tor if that is your threat).
...
Beem is designed to run on a home machine while being reachable from the internet via [Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/). This hides your home IP, terminates TLS on Cloudflare's edge, and gives you DDoS protection for free.
...
cloudflared tunnel create beem

# 4. point a DNS record at it (replace beem.example.com with your domain)
cloudflared tunnel route dns beem beem.example.com
```
...
- `https://beem.example.com` loads with a valid Cloudflare TLS cert.
- WebSocket connection upgrades to `wss://` automatically (the client picks `wss` when `location.protocol === "https:"`).
- `curl -I https://beem.example.com` response headers include Cloudflare's `cf-ray` and do **not** expose your origin IP anywhere.

## Roadmap

Beem is developed in numbered phases. Post-beta plans include:
